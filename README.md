...
| Запись события на приёме: ключ идемпотентности, `persist_event`, `202` после фиксации транзакции | В разработке |
...
| --- | --- |
| `X-Timestamp` | момент отправки в секундах Unix, допуск 300 секунд |
| `X-Signature-256` | HMAC-SHA256 в hex по строке `{timestamp}.{сырое тело}` |
...
> Событие пока не сохраняется: связка с записью `Event` и `OutboxMessage` в разработке. Сейчас `202` подтверждает только прохождение проверок.
